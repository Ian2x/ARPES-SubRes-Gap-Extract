...
for i in range(k_dim):
    ...
    temp_split = temp.split()
    k[k_dim-i-1] = -float(temp_split[0]) # flip to positive
    for j in range(w_dim):
        Eugen_data[w_dim - j - 1][i] = temp_split[j + 1] # flip to positive
...
for i in range(z_height):
    temp_w[z_height-1-i] = w[w_dim - 1 - i - height_offset]
    for j in range(z_width):
        Z[i][j] = Eugen_data[i + height_offset][j + width_offset]
        temp_k[z_width-1-j] = -k[w_dim - 1 - j - width_offset]
